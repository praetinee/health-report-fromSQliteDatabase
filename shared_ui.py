...
def clean_html_string(html_str):
    ...
    # ลบ leading/trailing whitespace ของแต่ละบรรทัด และต่อกันเป็น string เดียว
...
def inject_custom_css():
    ...
    <style>
        ...
        :root {
            --primary: #005f56;       /* Deep Teal - Professional & Trust */
        ...
        .stTabs [data-baseweb="tab"]:hover { color: var(--primary); background-color: rgba(0,95,86,0.05); }
        .stTabs [aria-selected="true"] {
            background-color: var(--primary) !important;
            color: white !important;
            box-shadow: 0 4px 12px rgba(0,95,86,0.3);
        }
        ...
        .lab-table, .info-detail-table { width: 100%; min-width: 300px; border-collapse: collapse; font-size: 0.95rem; }
        ...
        .abnormal-row { background-color: #fff5f5 !important; }
        ...
        .patient-profile { display: flex; gap: 20px; align-items: center; flex: 1 1 300px; }
        .profile-icon {
        ...
        .patient-name { font-size: 1.4rem; font-weight: 800; color: var(--text-main); margin-bottom: 4px; }
        .patient-meta { font-size: 0.95rem; color: var(--text-light); }
        .patient-dept {
            background: #f0f2f5; color: #555;
            padding: 4px 12px; border-radius: 6px;
            font-size: 0.85rem; font-weight: 600; display: inline-block; margin-top: 8px;
        }
        ...
        .vital-value { font-size: 1.25rem; font-weight: 800; color: var(--text-main); line-height: 1.2; }
        .vital-label { font-size: 0.85rem; color: var(--text-light); font-weight: 500; }
        .vital-sub { font-size: 0.8rem; color: #888; margin-top: 2px; }

        /* --- Result Cards (Hearing/Vision) --- */
        .result-card-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; }
        .result-card {
        ...
        .res-header { display: flex; align-items: center; gap: 12px; margin-bottom: 15px; border-bottom: 1px solid #f5f5f5; padding-bottom: 10px; }
        .res-icon { width: 40px; height: 40px; background: var(--primary-light); color: var(--primary); border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 1.2rem; }
        .res-title { font-weight: 700; font-size: 1.05rem; color: var(--text-main); }
        .res-status-badge {
            padding: 6px 14px; border-radius: 30px; font-size: 0.95rem; font-weight: 600; display: inline-block; margin-bottom: 12px;
        }
        .badge-normal { background: #e8f5e9; color: #2e7d32; }
        .badge-abnormal { background: #ffebee; color: #c62828; }
        .badge-warning { background: #fff3e0; color: #ef6c00; }
        
        .res-detail-row { display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 0.9rem; color: #555; padding: 8px; background: #fcfcfc; border-radius: 8px; }
...
def render_vision_details_table(person_data):
    ...
    def check_vision(val, test_type):
        if is_empty(val): return "-", "vision-not-tested", "bg-neutral"
        ...
        if val_str in normal_keywords: return "ปกติ", "vision-result vision-normal", "bg-green-light"
        if any(kw in val_str for kw in abnormal_keywords):
            if any(kw in val_str for kw in warning_keywords): return "ต่ำกว่าเกณฑ์", "vision-result vision-warning", "bg-orange-light"
            return "ผิดปกติ", "vision-result vision-abnormal", "bg-red-light"
        if any(kw in val_str for kw in warning_keywords): return "ต่ำกว่าเกณฑ์", "vision-result vision-warning", "bg-orange-light"
        if re.match(r'^\d+/\d+$', val_str): return str(val), "vision-result vision-normal", "bg-green-light"
        return str(val), "vision-result vision-normal", "bg-green-light"
    ...
    for item in vision_config:
        ...
        for key in item['keys']:
            if not is_empty(person_data.get(key)):
                ...
                any_data_found = True
                break
        res_text, res_class, _ = check_vision(val, item['id'])
        html_rows += f"<tr><td>{item['label']}</td><td class='result-cell' style='text-align:center;'><span class='{res_class}'>{res_text}</span></td></tr>"
...
def display_performance_report_hearing(person_data, all_person_history_df):
    ...
    def get_status_props(text):
        ...
        if "N/A" in txt or "ไม่ได้ตรวจ" in txt: return "badge-warning", "ไม่ได้ตรวจ"
    ...
    <div class="result-card-grid">
        ...
        <div class="result-card">
            ...
            <div class="res-detail-row">
                <span>🗣️ เสียงพูด (500-2k Hz)</span>
                <strong>{fmt_db(results['averages']['right_500_2000'])}</strong>
            </div>
            <div class="res-detail-row">
                <span>🔔 เสียงสูง (3k-6k Hz)</span>
                <strong>{fmt_db(results['averages']['right_3000_6000'])}</strong>
            </div>
        </div>
        ...
        <div class="result-card">
            ...
            <div class="res-detail-row">
                <span>🗣️ เสียงพูด (500-2k Hz)</span>
                <strong>{fmt_db(results['averages']['left_500_2000'])}</strong>
            </div>
            <div class="res-detail-row">
                <span>🔔 เสียงสูง (3k-6k Hz)</span>
                <strong>{fmt_db(results['averages']['left_3000_6000'])}</strong>
            </div>
        </div>
    </div>
    """)
