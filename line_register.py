...
def clean_string(val):
    """ลบช่องว่างหัวท้าย และแปลงเป็น string เพื่อความชัวร์"""
...
def normalize_db_name_field(full_name_str):
    ...
    โดยจัดการกับช่องว่างที่อาจจะเกินมาตรงกลาง หรือหัวท้าย
    ...
    if len(parts) >= 2:
        ...
        first_name = parts[0]
        last_name = " ".join(parts[1:]) # เผื่อกรณีมีชื่อกลาง หรือนามสกุลยาวๆ
...
def check_registration_logic(df, input_fname, input_lname, input_id):
    ...
    # สำคัญ: ต้องเทียบแบบ String เพื่อรักษาเลข 0 นำหน้า
    ...
    for index, row in user_match.iterrows():
        ...
        # เปรียบเทียบแบบ String Exact Match (หลังจาก Clean แล้ว)
...
def render_registration_page(df):
    ...
    # CSS ตกแต่งให้ดูเหมือนหน้า App มือถือ
    st.markdown("""
    <style>
        .reg-container {
    ...
    # เช็คว่าลงทะเบียนสำเร็จหรือยัง (State management)
    if st.session_state.get('line_register_success', False):
        ...
        # ตรงนี้ในอนาคตเราจะยิง API กลับไปหา LINE หรือบันทึก UserID ลง DB ถาวร
        return
    ...
    with st.container():
        ...
        # --- ส่วน PDPA (เอาอันเก่ามาปรับใช้ตามโจทย์) ---
        ...
        with st.form("line_reg_form"):
            ...
            # รับเป็น text เพื่อรักษาเลข 0 นำหน้า
