...
def interpret_bp(sbp, dbp): # THIS IS NOW GLOBAL
    try:
        sbp = float(sbp)
        dbp = float(dbp)
        if sbp == 0 or dbp == 0:
...
def combined_health_advice(bmi, sbp, dbp): # THIS IS NOW GLOBAL
...
def merge_final_advice_grouped(messages): # THIS IS NOW GLOBAL
    groups = {
...
def safe_value(val): # THIS IS NOW GLOBAL
    val = str(val or "").strip()
    if val.lower() in ["", "nan", "none", "-"]:
        return "-"
    return val

def interpret_alb(value): # THIS IS NOW GLOBAL
...
def interpret_sugar(value): # THIS IS NOW GLOBAL
    val = str(value).strip().lower()
    if val == "negative":
        return "ไม่พบ"
    elif val == "trace":
...
def parse_range_or_number(val): # THIS IS NOW GLOBAL
...
def interpret_rbc(value): # THIS IS NOW GLOBAL
...
def interpret_wbc(value): # THIS IS NOW GLOBAL
    val = str(value or "").strip().lower()
    if val in ["-", "", "none", "nan"]:
        return "-"
    low, high = parse_range_or_number(val)
...
def advice_urine(sex, alb, sugar, rbc, wbc): # THIS IS NOW GLOBAL
...
def is_urine_abnormal(test_name, value, normal_range): # THIS IS NOW GLOBAL
    val = str(value or "").strip().lower()
...
def render_urine_section(person_data, sex, year_selected): # THIS IS NOW GLOBAL
...
def interpret_cxr(val): # THIS IS NOW GLOBAL
    val = str(val or "").strip()
    if is_empty(val):
...
def get_ekg_col_name(year): # THIS IS NOW GLOBAL
    return "EKG" if year == 2568 else f"EKG{str(year)[-2:]}"

def interpret_ekg(val): # THIS IS NOW GLOBAL
    val = str(val or "").strip()
    if is_empty(val):
...
def safe_text(val): # THIS IS NOW GLOBAL
    return "-" if str(val).strip().lower() in ["", "none", "nan", "-"] else str(val).strip()

def hepatitis_b_advice(hbsag, hbsab, hbcab): # THIS IS NOW GLOBAL
    hbsag = hbsag.lower()
    hbsab = hbsab.lower()
    hbcab = hbcab.lower()
...
def render_stool_html_table(exam, cs): # THIS IS NOW GLOBAL
    style = """
    <style>
        .stool-container {
            background-color: var(--background-color);
...
def interpret_stool_exam(val): # THIS IS NOW GLOBAL
...
def interpret_stool_cs(value): # THIS IS NOW GLOBAL
...
# This uses the st.sidebar.form context
# ... (search form definition and logic) ...

# This part is at the global scope, it defines the sidebar elements first,
# then the main content based on session state.

# ==================== Main Content Area ====================
with st.container():
    # Only display the main report content if a person_row is successfully selected
    if "person_row" in st.session_state and st.session_state.get("selected_row_found", False):
        ...
        #=========================== ความเห็นแพทย์ =======================
        if "person_row" in st.session_state and st.session_state.get("selected_row_found", False):
            # This is already outside the main content block, but let's re-align it
            pass # Moved to bottom of main content block for clarity
        
        # Move the doctor's opinion and signature to the main display block (within this `if` scope)
