...
def inject_custom_css():
    ...
    <style>
        ...
        .transparent-border-table thead th {
...
def display_performance_report_lung(person_data):
    ...
    with main_col:
        ...
        st.markdown(df_details.to_html(escape=False, index=False, classes="styled-df-table transparent-border-table"), unsafe_allow_html=True)
...
" and the query is "- คำแนะนำหูกับปอดให้เป็นสีเหลืองโปร่งแสง
- ตารางผลปอดเอาเส้นกรอบล้อมรอบตารางออกเลย เส้นสีดำซ้าย ขวา ล่าง นั่นนะ เอาเป็นสีโปร่งใส".
I should edit the document.
The user wants to make two specific UI changes:
1.  **Change the advice box color for hearing and lung reports to a transparent yellow.**
    * In `display_performance_report_hearing`, the advice is displayed using `st.error`, `st.warning`, or `st.success`. The user wants this to be a consistent transparent yellow. I should change all of these to `st.warning`.
    * In `display_performance_report_lung`, the advice is displayed using `st.info`. I need to change this to `st.warning`.

2.  **Remove the outer border from the lung capacity details table.**
    * The table is rendered using `df_details.to_html(..., classes="styled-df-table")`.
    * I need to add a new CSS class that makes the left, right, and bottom borders transparent, while keeping the inner horizontal lines.
    * I'll create a class like `.transparent-border-table` in the `inject_custom_css` function.
    * Then, I'll apply this new class to the lung details table: `classes="styled-df-table transparent-border-table"`.

**Plan:**

1.  **Locate `display_performance_report_hearing` function.**
    * Find the `if/elif/else` block that displays the advice.
    * Change `st.error(...)` to `st.warning(...)`.
    * Change `st.success(...)` to `st.warning(...)`.
    * Keep `st.warning(...)` as is.

2.  **Locate `display_performance_report_lung` function.**
    * Find the line `st.info(lung_advice or "ไม่มีคำแนะนำเพิ่มเติม")`.
    * Change it to `st.warning(lung_advice or "ไม่มีคำแนะนำเพิ่มเติม")`.
    * Find the line `st.markdown(df_details.to_html(..., classes="styled-df-table"), ...)`.
    * Change the classes to `classes="styled-df-table transparent-border-table"`.

3.  **Locate `inject_custom_css` function.**
    * Add the new CSS rule for `.transparent-border-table`.
    ```css
    .transparent-border-table th, .transparent-border-table td {
        border-left-color: transparent;
        border-right-color: transparent;
    ...
    ```
    I will also need to adjust the existing `.styled-df-table th, .styled-df-table td` rule to ensure the bottom border is still there for other tables, but the new class will override the side borders for the lung table. The current rule is `border: 1px solid var(--border-color);`. The new rule will specifically target the left and right borders. I will also remove the top border of the header.

Let's implement the changes.รับทราบครับ ผมได้ปรับปรุงโค้ดใน Canvas ตามที่คุณต้องการเรียบร้อยแล้วครับ
...
def load_sqlite_data():
    ...
    try:
        file_id = "1HruO9AMrUfniC8hBWtumVdxLJayEc1Xr"
        download_url = f"[https://drive.google.com/uc?export=download&id=](https://drive.google.com/uc?export=download&id=){file_id}"
        response = requests.get(download_url)
        response.raise_for_status()
        with tempfile.NamedTemporaryFile(delete=False, suffix=".db") as tmp:
            tmp.write(response.content)
...
def display_common_header(person_data):
    ...
    <div class="vitals-grid">
        <div class="vital-card">
            <div class="vital-icon">
                <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"></path><path d="M12 6v6l4 2"></path></svg>
            </div>
            <div class="vital-data">
                <span class="vital-label">น้ำหนัก / ส่วนสูง</span>
                <span class="vital-value">{weight_val} kg / {height_val} cm</span>
        ...
        <div class="vital-card">
            <div class="vital-icon">
                <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"></polyline></svg>
            </div>
            <div class="vital-data">
                <span class="vital-label">ชีพจร (BPM)</span>
                <span class="vital-value">{pulse_val}</span>
    ...
    if not has_vision_data(person_data):
        ...
        if not is_empty(vision_advice_summary) or not is_empty(doctor_advice):
            ...
            if not is_empty(vision_advice_summary):
                 st.markdown(f"""
                 <div style='background-color: rgba(0, 123, 255, 0.1); border: 1px solid rgba(0, 123, 255, 0.2); padding: 1.25rem; border-radius: 0.75rem; margin-top: 1rem; display: flex; align-items: flex-start; gap: 1rem;'>
                    <div style='flex-shrink: 0;'><svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--primary-color)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg></div>
                    <div><h5 style='margin-top: 0; margin-bottom: 0.25rem; color: var(--primary-color);'>สรุปความเหมาะสมกับงาน</h5><p style='margin:0;'>{vision_advice_summary}</p></div>
                 </div>""", unsafe_allow_html=True)
            if not is_empty(doctor_advice):
                 st.markdown(f"""
