...
def render_urine_section(person_data, sex, year_selected):
    ...
    html_content = render_section_header("ผลการตรวจปัสสาวะ", "Urinalysis")
    html_content += "<div class='urine-table-container'><table class='urine-table' style='font-size:14px;width:100%;border-collapse:collapse;table-layout:fixed;'>"
    html_content += "<colgroup><col style='width:33.33%;'><col style='width:33.33%;'><col style='width:33.33%;'></colgroup>"
    ...
    if not has_any_urine_result: pass
        ...
        st.markdown(f"<div style='background-color:rgba(255,255,0,0.2);color:var(--text-color);padding:1rem;border-radius:6px;margin-top:1rem;font-size:14px;'>{summary}</div>", unsafe_allow_html=True)
    else:
        st.markdown("<div style='background-color:rgba(57,255,20,0.2);color:var(--text-color);padding:1rem;border-radius:6px;margin-top:1rem;font-size:14px;'>ผลตรวจปัสสาวะอยู่ในเกณฑ์ปกติ</div>", unsafe_allow_html=True)
...
def render_stool_html_table(exam, cs):
    html_content = "<div class='stool-container' style='margin-top:1rem;'><table class='stool-table' style='width:100%;border-collapse:collapse;table-layout:fixed;font-size:14px;'>"
    html_content += "<colgroup><col style='width:50%;'><col style='width:50%;'></colgroup>"
    html_content += f"<tr><th style='text-align:left;width:50%;'>ผลตรวจอุจจาระทั่วไป</th><td style='text-align:left;'>{exam if exam != '-' else 'ไม่ได้เข้ารับการตรวจ'}</td></tr>"
    html_content += f"<tr><th style='text-align:left;width:50%;'>ผลตรวจอุจจาระเพาะเชื้อ</th><td style='text-align:left;'>{cs if cs != '-' else 'ไม่ได้เข้ารับการตรวจ'}</td></tr>"
...
st.markdown("""
    ...
    div[data-testid="stWarning"], div[data-testid="stError"] { font-family: 'Sarabun', sans-serif !important; }
    @media print {
...
def update_report_view():
    """Updates the 'person_row' in session state based on the selected year and date."""
    try:
...
if st.sidebar.button("ค้นหา"):
    ...
    # Clear all session state except the search input value itself
    for key in list(st.session_state.keys()):
        if key != 'search_input_val':
    ...
    if not search_term:
        ...
        query_df = df[
            df["HN"].str.contains(search_term, na=False) |
            df["ชื่อ-สกุล"].str.contains(search_term, na=False)
        ]
        if query_df.empty:
            st.sidebar.error("❌ ไม่พบข้อมูล")
        else:
            st.session_state.search_result = query_df.reset_index(drop=True)
            # Find and set the most recent record as the default view
            latest_record = query_df.sort_values(by="Year", ascending=False).iloc[0]
            st.session_state.person_row = latest_record.to_dict()
            st.session_state.selected_row_found = True
            # Set initial values for selectboxes
...
# Sidebar Controls for Year/Date Selection
if "search_result" in st.session_state:
    ...
    with st.sidebar:
        ...
        st.selectbox(
            ...
            options=available_years, index=year_index, key="year_select_sidebar",
        ...
        if exam_dates_options:
            ...
            st.selectbox(
                "🗓️ เลือกวันที่ตรวจ",
                options=exam_dates_options, index=date_index, key="exam_date_select_sidebar",
...
if "person_row" in st.session_state:
    ...
    with st.container():
        ...
        with col_ua_right:
            ...
            cxr_col = get_ekg_col_name(person.get("Year")).replace("EKG", "CXR") # Simplified logic
            ...
            st.markdown(f"<div style='padding:1.25rem; border-radius:6px; margin-bottom:1.5rem; font-size:14px;'>{ekg_result}</div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<div style='padding:1rem; border-radius:6px; margin-bottom:1.5rem; font-size:14px;'>{hep_a_raw}</div>", unsafe_allow_html=True)
            ...
            st.markdown(f"""
                <div style="margin-bottom:1rem;">
                    <table style='width:100%;text-align:center;border-collapse:collapse;font-size:14px;'>
                        ...
                        <tbody><tr><td>{hbsag_raw}</td><td>{hbsab_raw}</td><td>{hbcab_raw}</td></tr></tbody>
                    </table>
                </div>
            """, unsafe_allow_html=True)
            ...
            st.markdown(f"<div style='padding:1rem 1.5rem; border-radius:6px; background-color:{bg_color_hep}; margin-bottom:1.5rem; font-size:14px;'>{advice}</div>", unsafe_allow_html=True)
