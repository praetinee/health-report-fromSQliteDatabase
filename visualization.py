...
def plot_health_radar(person_data):
    ...
    fig.add_trace(go.Scatterpolar(
        r=ideal_r, theta=categories, fill='toself', name='Ideal',
        line=dict(color='rgba(0, 200, 83, 0.3)', dash='dot', width=1),
        fillcolor='rgba(0, 200, 83, 0.05)', hoverinfo='skip'
    ))
...
def display_visualization_tab(person_data, history_df):
    ...
    with st.container(border=True):
        ...
        with c1:
            st.markdown("### 🛡️ ภาพรวมสุขภาพ")
            st.markdown("กราฟแสดงคะแนนสุขภาพ (เต็ม 100) สีเขียวคือดีเว้าแหว่งคือจุดเสี่ยง")
