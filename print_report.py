...
RECOMMENDATION_TEXTS_CBC = {
    "C2": "ให้รับประทานอาหารที่มีประโยชน์ เช่น นม ผักใบเขียว ถั่วเมล็ดแห้ง เนื้อสัตว์ไม่ติดมัน และพักผ่อนให้เพียงพอ",
    "C4": "ให้ดูแลสุขภาพให้แข็งแรง ออกกำลังกาย ทานอาหารที่มีประโยชน์ ระมัดระวังการเกิดเลือดออกผิดปกติ เช่น เลือดออกตามไรฟัน เลือดกำเดา",
    "C6": "ให้ดูแลสุขภาพร่างกายให้แข็งแรง หลีกเลี่ยงการอยู่ในที่ชุมชนที่มีการระบาดของโรค",
    "C8": "ให้นำผลตรวจพบแพทย์หาสาเหตุภาวะโลหิตจาง หรือกรณีรู้สาเหตุให้รับการรักษาต่อเนื่องจนกว่าระดับเลือดจะกลับสู่ภาวะปกติ",
    "C9": "ให้นำผลตรวจพบแพทย์หาสาเหตุภาวะโลหิตจาง หรือกรณีรู้สาเหตุให้รับการรักษาต่อเนื่องจนกว่าระดับเลือดจะกลับสู่ภาวะปกติ",
    "C10": "ให้นำผลตรวจพบแพทย์หาสาเหตุเกล็ดเลือดสูงกว่าปกติ หรือกรณีรู้สาเหตุให้รับการรักษาต่อเนื่องจนกว่าระดับเกล็ดเลือดจะกลับสู่ภาวะปกติ",
    "C13": "ให้รับประทานอาหารที่มีประโยชน์ เช่น นม ผักใบเขียว ถั่วเมล็ดแห้ง เนื้อสัตว์ไม่ติดมัน และพักผ่อนให้เพียงพอ",
}
# --- END OF CHANGE ---
...
RECOMMENDATION_TEXTS_URINE = {
    ...
    "E2": "อาจเกิดจากการปนเปื้อนในการเก็บปัสสาวะหรือมีการติดเชื้อในระบบทางเดินปัสสาวะ ให้ดื่มน้ำมากๆ และหลีกเลี่ยงการกลั้นปัสสาวะ",
}
# --- END OF CHANGE ---
...
def interpret_stool_exam(val):
    ...
    if "wbc" in val_lower or "เม็ดเลือดขาว" in val_lower:
...
def hepatitis_b_advice(hbsag, hbsab, hbcab):
    ...
    if "positive" in hbsag_logic:
        return "ติดเชื้อไวรัสตับอักเสบบี", "infection"
    if "positive" in hbsab_logic and "positive" not in hbsag_logic:
        return "มีภูมิคุ้มกันต่อไวรัสตับอักเสบบี", "immune"
    if "positive" in hbcab_logic and "positive" not in hbsab_logic:
        return "เคยติดเชื้อแต่ไม่มีภูมิคุ้มกันในปัจจุบัน", "no_immune"
    if all(x == "negative" for x in [hbsag_logic, hbsab_logic, hbcab_logic]):
        return "ไม่มีภูมิคุ้มกันต่อไวรัสตับอักเสบบี ควรรับวัคซีนให้ครบ", "no_immune"
...
def generate_fixed_recommendations(person_data):
    ...
    try:
        ...
        if fbs_val is not None and fbs_val > 100:
            recommendations.append("ตรวจพบน้ำตาลในเลือดสูงให้หลีกเลี่ยงการทานขนมหวาน /อาหารหวาน ออกกำลังกายสม่ำเสมอ และพบแพทย์เพื่อตรวจซ้ำ")
    except Exception:
        pass
    ...
    try:
        ...
        if lipid_str in ["ไขมันในเลือดสูง", "ไขมันในเลือดสูงเล็กน้อย"]:
            recommendations.append("ตรวจพบไขมันในเลือดสูงให้ลดอาหารไขมันสูงเช่น อาหารทอด/ไข่แดง และออกกำลังกายสม่ำเสมอ")
    except Exception:
        pass
    ...
    try:
        ...
        if liver_str in ["การทำงานของตับสูงกว่าเกณฑ์ปกติเล็กน้อย", "การทำงานของตับสูงกว่าเกณฑ์ปกติ"]:
            recommendations.append("ตรวจพบการทำงานของตับสูงเล็กน้อย งดสุรา/ลดอาหารไขมัน/อาหารปิ้งย่าง ตรวจซ้ำหรือตามนัดแพทย์")
    except Exception:
        pass
    ...
    try:
        ...
        if uric_str in ["กรดยูริคสูงกว่าเกณฑ์ปกติเล็กน้อย", "กรดยูริคสูงกว่าเกณฑ์ปกติ"]:
            recommendations.append("ตรวจพบกรดยูริคสูง (สาเหตุของโรคเก๊าท์) ลดอาหารเครื่องในสัตว์ ลดเครื่องดื่มแอลกอฮอล์")
    except Exception:
        pass
    ...
    try:
        ...
        if gfr_str == "การทำงานของไตต่ำกว่าเกณฑ์ปกติเล็กน้อย":
            recommendations.append("การทำงานของไตต่ำกว่าเกณฑ์ปกติเล็กน้อย ลดอาหารเค็ม อาหารโซเดียมสูง และดื่มน้ำให้เพียงพอ")
    except Exception:
        pass
...
def generate_cbc_recommendations(person_data, sex):
    ...
    if hb is not None:
        if sex == "ชาย":
            if hb < 12:
                status_ce = "พบภาวะโลหิตจาง"
            elif hb < 13:
                status_ce = "พบภาวะโลหิตจางเล็กน้อย"
            else:
                status_ce = "ความเข้มข้นของเลือดปกติ"
        elif sex == "หญิง":
            if hb < 11:
                status_ce = "พบภาวะโลหิตจาง"
            elif hb < 12:
    ...
    if wbc is not None:
        if 4000 <= wbc <= 10000:
            status_cf = "เม็ดเลือดขาวปกติ"
        elif 10000 < wbc < 13000:
            status_cf = "เม็ดเลือดขาวสูงกว่าเกณฑ์ปกติเล็กน้อย"
        elif wbc >= 13000:
            status_cf = "เม็ดเลือดขาวสูงกว่าเกณฑ์ปกติ"
        elif 3000 < wbc < 4000:
            status_cf = "เม็ดเลือดขาวต่ำกว่าเกณฑ์ปกติเล็กน้อย"
        elif wbc <= 3000:
            status_cf = "เม็ดเลือดขาวต่ำกว่าเกณฑ์ปกติ"

    status_cg = ""
    if plt is not None:
        if 150000 <= plt <= 500000:
            status_cg = "เกร็ดเลือดปกติ"
        elif 500000 < plt < 600000:
            status_cg = "เกร็ดเลือดสูงกว่าเกณฑ์ปกติเล็กน้อย"
        elif plt >= 600000:
            status_cg = "เกร็ดเลือดสูงกว่าเกณฑ์ปกติ"
        elif 100000 <= plt < 150000:
            status_cg = "เกร็ดเลือดต่ำกว่าเกณ���์ปกติเล็กน้อย"
        elif plt < 100000:
            status_cg = "เกร็ดเลือดต่ำกว่าเกณฑ์ปกติ"
...
def generate_urine_recommendations(person_data, sex):
    ...
    if is_empty(alb_raw):
        status_ct = ""
    elif alb_raw == "negative":
        status_ct = "ไม่พบโปรตีนในปัสสาวะ"
    elif alb_raw in ["trace", "2+", "1+"]:
        status_ct = "พบโปรตีนในปัสสาวะเล็กน้อย"
    elif alb_raw == "3+":
        status_ct = "พบโปรตีนในปัสสาวะ"

    status_cu = ""
    if is_empty(sugar_raw):
        status_cu = ""
    elif sugar_raw == "negative":
        status_cu = "ไม่พบน้ำตาลในปัสสาวะ"
    elif sugar_raw == "trace":
        status_cu = "พบน้ำตาลในปัสสาวะเล็กน้อย"
    elif sugar_raw in ["1+", "2+", "3+", "4+", "5+", "6+"]:
        status_cu = "พบน้ำตาลในปัสสาวะ"

    status_cv = ""
    if is_empty(rbc_raw):
        status_cv = ""
    elif rbc_raw in ["0-1", "negative", "1-2", "2-3", "3-5"]:
        status_cv = "เม็ดเลือดแดงในปัสสาวะปกติ"
    elif rbc_raw in ["5-10", "10-20"]:
        status_cv = "พบเม็ดเลือดแดงในปัสสาวะเล็กน้อย"
    else:
        status_cv = "พบเม็ดเลือดแดงในปัสสาวะ"

    status_cw = ""
    if is_empty(wbc_raw):
        status_cw = ""
    elif wbc_raw in ["0-1", "negative", "1-2", "2-3", "3-5"]:
        status_cw = "เม็ดเลือดขาวในปัสสาวะปกติ"
    elif wbc_raw in ["5-10", "10-20"]:
        status_cw = "พบเม็ดเลือดขาวในปัสสาวะเล็กน้อย"
    else:
        status_cw = "พบเม็ดเลือดขาวในปัสสาวะ"
    ...
    is_abnormal = False
    if status_ct == "พบโปรตีนในปัสสาวะ":
        is_abnormal = True
    if "น้ำตาล" in status_cu and "ไม่พบ" not in status_cu:
        is_abnormal = True
    if "เม็ดเลือดแดง" in status_cv and "ปกติ" not in status_cv:
        is_abnormal = True
    if status_cw == "พบเม็ดเลือดขาวในปัสสาวะ":
        is_abnormal = True
...
def generate_doctor_opinion(person_data, sex, cbc_statuses, urine_statuses):
    ...
    if fbs is not None:
        if 100 <= fbs < 106:
            opinion_parts.append("น้ำตาลในเลือดเริ่มสูงเล็กน้อย")
        elif 106 <= fbs < 126:
            opinion_parts.append("น้ำตาลในเลือดสูงเล็กน้อย")
        elif fbs >= 126:
            opinion_parts.append("น้ำตาลในเลือดสูง")
    ...
    if cbc_statuses.get('status_ce') == "��บภาวะโลหิตจาง":
        opinion_parts.append("พบภาวะโลหิตจาง")

    if cbc_statuses.get('status_cf') in [
        "เม็ดเลือดขาวสูงกว่าเกณฑ์ปกติ",
        "เม็ดเลือดขาวต่ำกว่าเกณฑ์ปกติ",
        "เม็ดเลือดขาวสูงกว่าเกณฑ์ปกติเล็กน้อย",
        "เม็ดเลือดขาวต่ำกว่าเกณฑ์ปกติเล็กน้อย",
    ]:
        opinion_parts.append("เม็ดเลือดขาวผิดปกติ")

    if cbc_statuses.get('status_cg') in [
        "เกร็ดเลือดสูงกว่าเกณฑ์ปกติ",
        "เกร็ดเลือดต่ำกว่าเกณฑ์ปกติ",
        "เกร็ดเลือดสูงกว่าเกณฑ์ปกติเล็กน้อย",
        "เกร็ดเลือดต่ำกว่าเกณฑ์ปกติเล็กน้อย",
    ]:
...
def render_section_header(title, subtitle=None):
    ...
    return f"""
    <div style='background-color: #f0f2f6; color: #333; text-align: center; padding: 0.2rem 0.4rem; font-weight: bold; border-radius: 6px; margin-top: 0.5rem; margin-bottom: 0.2rem; font-size: 11px; border: 1px solid #dfe3eb;'>
        {full_title}
    </div>
    """
...
def render_header_and_vitals(person_data):
    ...
    age = str(int(float(person_data.get('อายุ')))) if str(person_data.get('อายุ')).replace('.', '', 1).isdigit() else person_data.get('อา���ุ', '-')
...
def render_lab_section(person, sex, cbc_statuses):
    ...
    cbc_config = [
        ...
        ("จำนวนเม็ดเลือดแดง (RBC)", "RBC (million)", "4.2 - 5.9 ล้านเซลล์/µl", 4.2, 5.9),
        ("จำนวนเม็ดเลือดขาว (WBC)", "WBC (cumm)", "4,000 - 10,000 เซลล์/µl", 4000, 10000),
        ("เกร็ดเลือด (Platelet)", "Plt (/mm)", "150,000 - 500,000 /mm³", 150000, 500000),
    ]
    cbc_rows = []
    ...
    blood_config = [
        ...
        ("การทำงานของไต (BUN)", "BUN", "7 - 20 mg%", 7, 20, False),
        ("ครีเอตินิน (Creatinine)", "Creatinine", "0.6 - 1.3 mg%", 0.6, 1.3, False),
        ("โคเลสเตอรอล (Cholesterol)", "CHOL", "< 200 mg%", None, 200, False),
        ("ไตรกลีเซอไรด์ (Triglyceride)", "TGL", "< 150 mg%", None, 150, False),
        ("ไขมันดี (HDL)", "HDL", "> 40 mg%", 40, None, True),
        ("ไขมันเลว (LDL)", "LDL", "< 130 mg%", None, 130, False),
        ("เอนไซม์ตับ (SGOT)", "SGOT", "<= 36 U/L", None, 36, False),
        ("เอนไซม์ตับ (SGPT)", "SGPT", "<= 40 U/L", None, 40, False),
        ("ALP", "ALP", "<= 120 U/L", None, 120, False),
        ("GFR", "GFR", ">= 60 ml/min/1.73m²", 60, None, True),
    ]
    blood_rows = []
    for label, col, norm, low, high, higher_is_better in blood_config:
        val = get_float(col, person)
        result, is_abn = flag(val, low, high, higher_is_better=higher_is_better)
...
def render_other_results_html(person, sex, urine_statuses, doctor_opinion, all_person_history_df=None):
    urine_data = [
        ...
        ("เม็ดเลือดแดง (RBC)", "RBC1", "0-5 cell/hpf"),
        ("เม็ดเลือดขาว (WBC)", "WBC1", "0-5 cell/hpf"),
        ("Epithelial cell", "Epi", "0-5 cell/hpf"),
        ("Bacteria", "Bacteria", "Negative"),
        ("Casts", "Casts", "Negative"),
        ("Crystals", "Crystal", "Negative"),
    ]
    urine_rows = []
...
def get_main_report_css():
    ...
    <style>
        ...
        body { 
            ...
            font-size: 12px;            /* was 9.5px */
            margin: 0.5cm;
            color: #333; 
            background-color: #fff; 
        }

        p, div, span, td, th { line_height: 1.5; }
        ...
        .print-lab-table th { 
            ...
            font-weight: bold; 
            font-size: 12px;            /* slightly larger headers */
        }
        ...
        .print-lab-table tfoot li { margin-bottom: 3px; }
        
        .header-grid { display: flex; align-items: flex-end; justify-content: space-between; margin-bottom: 0.6rem; }
        .header-left { text-align: left; }
        .header-right { text-align: right; }
        .info-table { font-size: 11px; text-align: left; }
        .info-table td { padding: 2px 6px; border: none; }
        
        .advice-box { padding: 0.6rem 1.1rem; border-radius: 8px; line-height: 1.6; margin-top: 0.6rem; border: 1px solid #ddd; page-break-inside: avoid; }
        .advice-title { font-weight: bold; margin-bottom: 0.35rem; font-size: 12px; }
        .advice-content ul { padding-left: 20px; margin: 0; }
        .advice-content ul li { margin-bottom: 5px; }
        
        .doctor-opinion-box {
            background-color: #e8f5e9;
            border-color: #a5d6a7;
            border: 1px solid #ddd;
            padding: 0.2rem 0.7rem;     /* a bit more padding */
            border-radius: 8px;
            line-height: 1.6;
            margin-top: 0.6rem;
        ...
        .perf-section { margin-top: 0.6rem; page-break-inside: avoid; border: 1px solid #e0e0e0; border-radius: 8px; padding: 0.6rem; }
        .summary-box { background-color: #f8f9fa; border-radius: 4px; padding: 6px 10px; margin-top: 4px; font-size: 11px; }
        @media print { 
